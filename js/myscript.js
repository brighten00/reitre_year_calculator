...
function errorReact() {
  if (errorTime == 4) {
    document.getElementsByClassName("conversation")[0].innerHTML =
      "Are you kidding me right now?";
    document
      .getElementById("character-picture")
      .setAttribute("src", "img/s_angry1.png");
  }
  if (errorTime == 5) {
    document.getElementsByClassName("conversation")[0].innerHTML =
      "You serious?";
    ...
    document
      .getElementById("character-picture")
      .setAttribute("src", "img/s_angry2.png");
  }
  if (errorTime > 5) {
    document
      .getElementById("character-picture")
...
function calculateResult() {
  ...
  var retireExpense = parseInt(
    document.getElementById("retired-expense").value
  );
  ...
  var yearInvestment = parseInt(
    document.getElementById("year-investment").value
  );
  ...
  var retirePension = parseInt(
    document.getElementById("retired-pension").value
  );
  ...
  var currentInvestment = parseInt(
    document.getElementById("current-investment").value
  );
  ...
  var x = (expectedLife + currentAge) / 2;
  var y = expectedLife - x;
  var bottomLine = currentAge;
  var topLine = expectedLife;
  var monthSaving = monthIncome - monthExpense - yearInvestment / 12;
  if (monthSaving < 0) {
    document.getElementsByClassName("conversation")[0].innerHTML =
      "Your income and spending do not match with each other.";
    errorTime = errorTime + 1;
    document
      .getElementById("character-picture")
      .setAttribute("src", "img/s_ken.png");
    return false;
  }
  var retireRemain = retireExpense - retireIncome;
  if (retireRemain < 0) {
    document.getElementsByClassName("conversation")[0].innerHTML =
      "Looks like you can still have a stable income after retirement. How about retire now?";
    document
      .getElementById("character-picture")
      .setAttribute("src", "img/s_ure.png");
    document.getElementById("css").setAttribute("href", "css/style.css");
    errorTime = 0;
    return false;
  }
  var calculateLimit = 0;
  do {
    var returnFactor = 1;
    if (irr == 0) {
      returnFactor = 1;
      ...
      returnFactor =
        (1 - Math.pow(1 + irr / 100, x - currentAge)) / (1 - (1 + irr / 100));
    }
    equationLeft =
      monthSaving * x * 12 +
      retirePension +
      currentInvestment * Math.pow(1 + irr / 100, x) +
      yearInvestment * returnFactor +
      currentSaving;
    ...
    if (x > expectedLife - 1) {
      calculateLimit = calculateLimit + 1;
    }
    if (calculateLimit > 10) {
      break;
    }
  ...
  if (x < 45) {
    document.getElementsByClassName("conversation")[0].innerHTML =
      "Looks like you can live a easy and relax life.";
    document
      .getElementById("character-picture")
      .setAttribute("src", "img/s_normal2.png");
  }
  if (x > 70) {
    document.getElementsByClassName("conversation")[0].innerHTML =
      "Maybe it is time to find a new job or re-examine your life style.";
    document
      .getElementById("character-picture")
      .setAttribute("src", "img/s_kon.png");
  }
  if (x == expectedLife) {
    document.getElementsByClassName("conversation")[0].innerHTML =
      "Looks like you don't have retired life QQ.";
    document
      .getElementById("character-picture")
      .setAttribute("src", "img/s_kon.png");
  }
  if (irr > 70) {
    document.getElementsByClassName("conversation")[0].innerHTML =
      "Have you ever considered to be a professional trader?";
    document
      .getElementById("character-picture")
      .setAttribute("src", "img/s_gimonn.png");
  }
  document.getElementById("css").setAttribute("href", "css/style.css");
  errorTime = 0;
}
